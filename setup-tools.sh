...
# setup for AWS cli
aws sts get-caller-identity --query Arn | grep soup-terraform-admin > /dev/null
if [ $? -eq 0 ]; then
...
if [ ! `which ecsctl 2> /dev/null`]; then
    echo "Install eksctl"
...
cd ~/environment/soup-tf-code/lb
